...
// ============================================================================
// CONFIGURACIÓN DE LA EMPRESA — EDITAR AQUÍ CON LOS DATOS REALES
// ============================================================================
const EMPRESA = {
    nombre: 'GOYO MOTOR\'S',                          // [EDITAR] Razón social a mostrar
    ruc: '[EDITAR: RUC AQUÍ]',                        // [EDITAR] RUC
    email: '[EDITAR: EMAIL@GOYOMOTORS.COM]',          // [EDITAR] Email de contacto
    direccion: '[EDITAR: DIRECCIÓN COMPLETA]',        // [EDITAR] Dirección de la tienda
    telefono: '[EDITAR: TELÉFONO]',                   // [EDITAR] Teléfono de contacto
    // IMPORTANTE: este logo debe copiarse a la carpeta /public de tu proyecto Next.js
    // (ej: public/logo.png), NO debe quedarse dentro de /components/utils.
    // fetch() solo puede leer archivos servidos como estáticos públicos.
    logoPath: '/logo.png',                            // Ruta del logo en /public
};
...
const drawProfessionalTable = (pdf, data, headers, colWidths, startX, startY, fontName, pageHeight, margin, rowMeta = []) => {
    let currentY = startY;
...
const generarPDFVenta = async (ventaData, clienteData = null) => {
    try {
        ...
        // COLUMNA IZQUIERDA
        pdf.text(`R.U.C: ${EMPRESA.ruc}`, margin, currentY);
        pdf.text(`EMAIL: ${EMPRESA.email}`, margin, currentY + 4);

        // COLUMNA DERECHA
        pdf.text(`DIRECCION: ${EMPRESA.direccion}`, pageWidth / 2, currentY);
        pdf.text(`TELEFONO: ${EMPRESA.telefono}`, pageWidth / 2, currentY + 4);

        currentY += 13;

        // Estado de devolución de la venta completa (badge visual junto al estado)
        const estadoDevVenta = ventaData.estadoDevolucion; // 'devuelta' | 'parcial' | undefined
        ...
        drawInfoLine(pdf, fontName, 'TIPO DE VENTA: ', getTipoVentaLabel(ventaData.tipoVenta), pageWidth / 2, currentY);
        currentY += 5;
        ...
        // Estado: si tiene devolución, mostrar eso en vez del estado base
        let estadoTexto;
        ...
        // Medir si "METODO DE PAGO: <texto>" se mete en el territorio de la columna derecha.
        // Si es pago mixto o el texto es largo, Estado pasa a su propia línea para no cruzarse.
        pdf.setFont(fontName, 'normal');
        ...
        if (esPagoMixto || anchoMetodoPago > espacioDisponible) {
            // Método de pago ocupa toda la línea, Estado va debajo
            ...
            // Caben ambos en la misma línea sin cruzarse
            drawInfoLine(pdf, fontName, 'METODO DE PAGO: ', metodoPagoTexto, margin, currentY);
            drawInfoLine(pdf, fontName, 'ESTADO: ', estadoTexto, pageWidth / 2, currentY);
            currentY += 5;
        }
        ...
        const dni = clienteData?.dni || ventaData.clienteDNI;
        if (dni) {
            drawInfoLine(pdf, fontName, 'DNI: ', String(dni), margin, currentY);
            currentY += 5;
        }

        if (ventaData.empleadoId) {
            drawInfoLine(pdf, fontName, 'REGISTRADO POR: ', ventaData.empleadoId, margin, currentY);
            currentY += 5;
        }
        ...
        currentY = drawProfessionalTable(pdf, tableData, tableHeaders, colWidths, margin, currentY, fontName, pageHeight, margin, rowMeta);
        ...
        if (ventaData.tipoVenta === 'credito' && abonosVenta.length > 0) {
            ...
            abonosVenta.forEach(abono => {
                if (currentY + 5 > pageHeight - margin) { pdf.addPage(); currentY = margin; }
        ...
        if (devolucionesVenta.length > 0) {
            ...
            devolucionesVenta.forEach(dev => {
                if (currentY + 12 > pageHeight - margin) { pdf.addPage(); currentY = margin; }
                ...
                (dev.items || []).forEach(item => {
                    if (currentY + 4 > pageHeight - margin) { pdf.addPage(); currentY = margin; }
        ...
        if (devolucionesVenta.length > 0 || (ventaData.tipoVenta === 'credito' && abonosVenta.length > 0)) {
            if (currentY + 15 > pageHeight - margin) { pdf.addPage(); currentY = margin; }
